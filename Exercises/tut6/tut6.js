...
function main() {
    ...
    let direction = vec2.fromValues(1.0, 1.0);
    var then = 0;
    var test = 0;
    loop(); 
    ...
    function draw(deltaTime) { 
        getReadyToDraw();
        drawTriangle(deltaTime);
    }; 

    function getReadyToDraw() { 
        gl.clearColor(0., 0., 0., 0.); // NOTE: This is the background color.
    ...
    function drawTriangle(dt) { 
        ...
        let p1 = vec2.fromValues(0.5, 0.);
        let p2 = vec2.fromValues(0.25, 0.5);
        points.push(p0);
        points.push(p1);
        points.push(p2);
